...
function showAllData() {
  ...
  products.forEach(function (data) {
    var ele = `
      <div class="product">
        <img src=${data.image.replace('&quot;', '')} alt="${data.title}">
        <h3>${data.title}</h3>
        <p>${data.category}</p>
    ...
    cart.forEach(function (data) {
      var ele = `
        <div class="cart-item">
          ...
          <p>Rating: ${data.rating.rate}</p>
          <p>Price: $${data.price}</p>
          <input type="number" min="1" value="${data.quantity}" onchange="updateQuantity(${data.id}, this.value)">
          <button onclick="removeFromCart(${data.id})">Remove</button>
        </div>`;
      cartDiv.innerHTML += ele;
    });
    cartDiv.innerHTML += `<p>Total Price: $${calculateTotalPrice().toFixed(2)}</p>`;
    cartDiv.innerHTML += `<button onclick="clearCart()">Clear Cart</button>`;
  }
}
